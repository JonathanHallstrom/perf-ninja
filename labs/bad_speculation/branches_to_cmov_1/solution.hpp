...
public:
    ...
    void simulateNext() {
        ...
        auto countNeighbours = [&](int i, int j) {
            ...
            return aliveNeighbours - current[i][j];
        };

        auto countNeighboursUnchecked = [&](int i, int j) {
            int aliveNeighbours = 0;
            // finding the number of neighbours that are alive
            for (int p = -1; p <= 1; p++) {     // row-offset (-1,0,1)
                for (int q = -1; q <= 1; q++) { // col-offset (-1,0,1)
                    aliveNeighbours += current[i + p][j + q];
                }
            }
        ...
        auto process = [&](int i, int j) {
            int aliveNeighbours = i > 0 && j > 0 && i < M - 1 && j < N - 1 ? countNeighboursUnchecked(i, j) : countNeighbours(i, j);

            // Implementing the Rules of Life:
            int val = current[i][j];
            val |= aliveNeighbours == 3;
            val &= (aliveNeighbours >= 2) & (aliveNeighbours <= 3);
            future[i][j] = val;
        };
        for (int i = 0; i < M; i++) {
            for (int j = 0; j < N; j++) {
